...
- **Config location** follows XDG. `meeko.config.resolve_config_path()` resolves it by precedence: `$MEEKO_CONFIG` → `$XDG_CONFIG_HOME/meeko/meeko.toml` (i.e. `~/.config/meeko/meeko.toml`) → `./meeko.toml` (cwd fallback, kept for dev). On first run `ensure_config_exists()` copies the tracked `meeko.toml.example` into the XDG location, so a fresh checkout starts with working defaults — no manual copy step. Do not re-track `meeko.toml` (still gitignored for the cwd-fallback dev case). The DB lives separately in the XDG data dir (`meeko.sessions.default_db_path()`, `~/.local/share/meeko/`).
